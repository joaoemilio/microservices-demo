...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	ecsLogger.Infof("[PlaceOrder] user_id=%q user_currency=%q", req.UserId, req.UserCurrency)
	...
	span, ctx := apm.StartSpan(ctx, "calculate_charge", "external")
	...
	ecsLogger.Infof("payment went through (transaction_id: %s)", txID)
	...
	if err := cs.sendOrderConfirmation(ctx, req.Email, orderResult); err != nil {
		e := apm.CaptureError(ctx, err)
		e.Send()
		ecsLogger.Warnf("failed to send order confirmation to %q: %+v", req.Email, err)
	} else {
		ecsLogger.Infof("order confirmation email sent to %q", req.Email)
	}
...
func (cs *checkoutService) getUserCart(ctx context.Context, userID string) ([]*pb.CartItem, error) {
	span, ctx := apm.StartSpan(ctx, "getUserCart", "external")
...
func (cs *checkoutService) prepOrderItems(ctx context.Context, items []*pb.CartItem, userCurrency string) ([]*pb.OrderItem, error) {
	...
	for i, item := range items {
		p_span, ctx := apm.StartSpan(ctx, "GetProduct", "external")
		...
		defer p_span.End()
		c_span, ctx := apm.StartSpan(ctx, "convertCurrency", "external")
...
func (cs *checkoutService) quoteShipping(ctx context.Context, address *pb.Address, items []*pb.CartItem) (*pb.Money, error) {
	span, ctx := apm.StartSpan(ctx, "quoteShipping", "external")
...
func (cs *checkoutService) convertCurrency(ctx context.Context, from *pb.Money, toCurrency string) (*pb.Money, error) {
	span, ctx := apm.StartSpan(ctx, "convertCurrency", "external")
...
func (cs *checkoutService) sendOrderConfirmation(ctx context.Context, email string, order *pb.OrderResult) error {
	span, ctx := apm.StartSpan(ctx, "sendOrderConfirmation", "external")
...
func (cs *checkoutService) shipOrder(ctx context.Context, address *pb.Address, items []*pb.CartItem) (string, error) {
	span, ctx := apm.StartSpan(ctx, "shipOrder", "external")
	span_ctx := apm.DestinationServiceSpanContext{ Name: "grpc", Resource: cs.shippingSvcAddr }
