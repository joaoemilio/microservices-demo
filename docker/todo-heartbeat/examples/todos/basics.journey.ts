...
journey("synthetics - check that title is present", async ({page}) => {
    step("go to app", async () => {
        const path = 'http://todo:8080/index.html';
...
journey("synthetics - check that input placeholder is correct", async ({page}) => {
    step("go to app", async () => {
        const path = 'http://todo:8080/index.html';
        await page.goto(path);
    })
