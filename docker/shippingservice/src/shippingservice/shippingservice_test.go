...
func TestGetQuote(t *testing.T) {
	...
	defer conn.Close()
	tx := apm.DefaultTracer.StartTransaction("/hipstershop.ShippingService/GetQuote", "request")
...
func TestShipOrder(t *testing.T) {
	...
	defer conn.Close()
	tx := apm.DefaultTracer.StartTransaction("/hipstershop.ShippingService/ShipOrder", "request")
	ctx := apm.ContextWithTransaction(context.Background(), tx)
	defer tx.End()
	client := pb.NewShippingServiceClient(conn)
	// A basic test case to test logic and protobuf interactions.
	...
	res, err := client.ShipOrder(ctx, req)
	if err != nil {
