...
func readCatalogFile(catalog *pb.ListProductsResponse, ctx context.Context) error {
	...
	if err := jsonpb.Unmarshal(bytes.NewReader(catalogJSON), catalog); err != nil {
		ecsLogger.Warnf("failed to parse the catalog JSON: %v", err)
		e := apm.CaptureError(ctx, err)
		e.Send()
		return err
	}
	ecsLogger.Info("successfully parsed product catalog json")
	return nil
}

func parseCatalog(ctx context.Context) []*pb.Product {
	span, ctx := apm.StartSpan(ctx, "parseCatalog", "parse")
	if reloadCatalog || len(cat.Products) == 0 {
		err := readCatalogFile(&cat, ctx)
		if err != nil {
			return []*pb.Product{}
		}
	}
...
func (p *productCatalog) ListProducts(ctx context.Context, empty *pb.Empty) (*pb.ListProductsResponse, error) {
	...
	ecsLogger.Info("listing products")
	time.Sleep(extraLatency)
	catalogue := &pb.ListProductsResponse{Products: parseCatalog(ctx)}
	defer span.End()
...
func (p *productCatalog) GetProduct(ctx context.Context, req *pb.GetProductRequest) (*pb.Product, error) {
	...
	ecsLogger.Info("Getting product with ID ", req.Id )
	...
	ecsLogger.Info("Found product with ID ", req.Id )
	defer span.End()
	return found, nil
}
